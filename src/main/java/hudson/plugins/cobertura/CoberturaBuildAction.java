 ...
 */
public class CoberturaBuildAction implements HealthReportingAction,
		StaplerProxy, Chartable {
	...
	private boolean copyHealth;
	private String neighbourJobName;
	/**
	...
	public HealthReport getBuildHealth() {
		...
		// try to get targets from root project (for maven modules targets are
		// null)
		DescribableList rootpublishers = owner.getProject().getRootProject()
				.getPublishersList();

		if (rootpublishers != null) {
			CoberturaPublisher publisher = (CoberturaPublisher) rootpublishers
					.get(CoberturaPublisher.class);
			if (publisher != null) {
		...
		Map<CoverageMetric, Integer> scores = healthyTarget.getRangeScores(
				unhealthyTarget, result);
		int minValue = 100;
		...
		if (minKey == null) {
			if (result == null || result.size() == 0) {
				...
				if (minKey != null) {
					Localizable localizedDescription = Messages
							._CoberturaBuildAction_description(
									result.get(minKey).getPercentage(), result
											.get(minKey).toString(), minKey
											.getName());
			...
			Localizable localizedDescription = Messages
					._CoberturaBuildAction_description(result.get(minKey)
							.getPercentage(), result.get(minKey).toString(),
							minKey.getName());
			health = new HealthReport(minValue, localizedDescription);
			return health;
		}
	...
	public String getIconFileName() {
		return "graph.gif"; // To change body of implemented methods use File |
	...
	public String getDisplayName() {
		return Messages.CoberturaBuildAction_displayName(); // To change body of
															// implemented
															// methods use File
															// | Settings | File
															// Templates.
	...
	public String getUrlName() {
		return "cobertura"; // To change body of implemented methods use File |
							// Settings | File Templates.
	}
	...
	public Object getTarget() {
		return getResult(); // To change body of implemented methods use File |
							// Settings | File Templates.
	}
	...
	static CoberturaBuildAction getPreviousResult(AbstractBuild<?, ?> start) {
		...
		while (true) {
			...
			if (r != null && r.includeOnlyStable()
					&& b.getResult() != Result.SUCCESS) {
				r = null;
			}
			if (r != null) {
				return r;
	...
	CoberturaBuildAction(AbstractBuild<?, ?> owner, CoverageResult r,
		...
		getBuildHealth(); // populate the health field so we don't have to parse
	...
	private static final Logger logger = Logger
			.getLogger(CoberturaBuildAction.class.getName());

	public static CoberturaBuildAction load(AbstractBuild<?, ?> build,
			CoverageResult result, CoverageTarget healthyTarget,
			CoverageTarget unhealthyTarget, boolean onlyStable,
			boolean failUnhealthy, boolean failUnstable,
			boolean autoUpdateHealth, boolean autoUpdateStability,
			boolean copyHealth, String neighbourJobName) {
		return new CoberturaBuildAction(build, result, healthyTarget,
				unhealthyTarget, onlyStable, failUnhealthy, failUnstable,
				autoUpdateHealth, autoUpdateStability, copyHealth,
				neighbourJobName);
	}
